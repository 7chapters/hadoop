create jar files of the project
...
store url_review_distinct into '/tmp/url_review_distinct/'  ;
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
LOCATION '/tmp/url_review_distinct/';

LOAD DATA INPATH '/tmp/url_review_distinct/part-r-00000' into table URL_REVIEW_distinct;

select * from url_review_distinct;
...
select * from url_cat u join url_review_distinct r  on u.hash = r.hash where u.category = 'political';

---------------------------------------------------------------------------------------------

insert overwrite  directory '/tmp/review/reviewfinal/'
